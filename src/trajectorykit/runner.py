...
def _finalize(state: AgentState, final_content: str) -> Dict[str, Any]:
    ...
    if getattr(state, "rubric", None):
        state.episode.rubric = state.rubric
    return {
...
def _inject_pre_turn(state: AgentState) -> Optional[List[dict]]:
    ...
    # ── Rubric generation (root only, turn 1 — before research begins) ─
    if (state.depth == 0
            and state.turn == 1
            and state.rubric is None
            and getattr(_cfg, "RUBRIC_ENABLED", False)):
        state.rubric = _generate_rubric(state)
        if state.rubric:
            state.messages.append({"role": "system", "content": (
                "📐 RESEARCH QUALITY RUBRIC (generated by external critic):\n"
                "This rubric defines what an excellent answer looks like for this question. "
                "Use it to plan your research tasks and ensure full coverage before drafting.\n\n"
                f"{state.rubric}\n\n"
                "Ensure every coverage checklist item is addressed in your research plan. "
                "The rubric will also be used to evaluate your draft."
            )})
            if state.verbose:
                print(f"📐  Injected rubric into planning context")
